  ...
  constructor(
    ...
    */
     ...
     this.dataService.postAction('query', {query: payload})
              .map((data) => data.bot_response )
              .subscribe( response => {
                console.log('bot chat response: ');
                console.log(response);
                this.store.dispatch({type: 'NEW_MESSAGE', payload: response.result.fulfillment.speech});
                this.store.dispatch({type: 'SHOW_VISUAL', payload: response});
              });
   }
